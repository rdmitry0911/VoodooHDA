...
//	logMsg("VoodooHDADevice[%p]::getCapabilities\n", this);
	...
	/* Intel Wellsburg (X99) errata: RIRB DMA silently fails when RIRBLBASE==0,
	 * which happens if the physical address falls on a 4GB boundary.  Force
	 * 32-bit DMA so RIRBLBASE is always non-zero. */
	if (mDeviceId == HDA_INTEL_WELLSBURG1 || mDeviceId == HDA_INTEL_WELLSBURG2)
		mSupports64Bit = false;
